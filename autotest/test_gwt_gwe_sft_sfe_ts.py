"""SFT/SFE PERIOD time-series, PERIOD AUXILIARY, and cross-period
persistence integration tests.

Covers GWT-SFT and GWE-SFE in paired cases sharing the same GWF+SFR model.

  PERIOD TS: CONCENTRATION/TEMPERATURE (CONSTANT status) and RAINFALL
             linked to a STEPWISE TS that steps mid-period; both must
             track the new value every timestep.
  ...
  Cross-period persistence: a PERIOD INFLOW value linked to a TS keeps
             tracking it into a later period whose own PERIOD block
             reappears (for STATUS/CONCENTRATION) without repeating
             INFLOW.
...
def _build_ts_shell(name, ws, thermal):
    """TDIS + GWF-SFR + GWT/GWE shell shared by the ts/peraux cases."""
...
def _build_cont_shell(name, ws, thermal):
    """TDIS + GWF-SFR + GWT/GWE shell shared by the persistence cases."""
    nlay, nrow, ncol, delr, delc, top, botm, idomain = _grid_params()
    aux_name = "TEMPERATURE" if thermal else "CONCENTRATION"
...
def _check_ts(bud_fname, obs_fname, pkg_label, conc_col, rain_col, thermal=False):
    """Shared T1+T2 check for SFT and SFE: verifies both the generic
    PERIOD TS re-sync and RAINFALL's own per-timestep re-sync."""
    # ------------------------------------------------------------------ T1 --
