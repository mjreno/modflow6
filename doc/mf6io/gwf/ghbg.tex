Input to the General-Head Boundary (GHBG) Package is read from the file that has type ``GHB6'' in the Name File.  Any number of GHB6 packages can be specified for a single groundwater flow model.  The package type in the GWF Model name file PACKAGES block is always ``GHB6'' regardless of whether list-based or grid array-based input is used; the READARRAYGRID option within the package input file activates grid array-based input.

When grid array-based input is used for general-head boundaries, the DIMENSIONS block is optional.  The input array size is determined from the model grid.  Cells that define no data should contain the DNODATA (3.0E+30) value.  When defined, the MAXBOUND dimension represents the maximum number of defined general-head boundary (non-DNODATA) cells found in any stress period.  Defining MAXBOUND can reduce the memory footprint of the grid array-based GHBG Package, especially when the input grid arrays are sparse and the model grid is large.  Grid array-based input for general-head boundaries can be used with the DIS and DISV Packages.
