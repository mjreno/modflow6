"""MWT/MWE PERIOD time-series, PERIOD AUXILIARY, and cross-period
persistence integration tests.

Covers GWT-MWT and GWE-MWE in paired cases sharing the same GWF+MAW model.

  PERIOD TS: CONCENTRATION/TEMPERATURE and RATE linked to a STEPWISE TS
             that changes mid-period; both must track the new value.
  PACKAGEDATA AUX TS: AUX1 linked to a TS; the budget AUXILIARY term
             must reflect its per-timestep value.
  PERIOD AUXILIARY literal: overrides the PACKAGEDATA AUX baseline
             (99.0 -> 55.0).
  Cross-period persistence: a PERIOD setting issued once must persist
             into a later period whose own PERIOD block doesn't repeat
             it -- checked for AUXILIARY, RATE, and CONCENTRATION/
             TEMPERATURE.
  AUX/PACKAGEDATA-TS collision: a literal PERIOD AUXILIARY override
             must not be silently overwritten by a still-active
             PACKAGEDATA AUX time series on the next timestep.
...
Cases:
  ...
  mwtauxovr    -- literal AUXILIARY override outlives an active
                  PACKAGEDATA AUX TS, GWT-MWT
...
def _build_gwf(
    ...
    flopy.mf6.ModflowGwfmaw(
        ...
        # injection (qbnd > 0): RATE conc/temp applies to the flow
        perioddata=[[0, "rate", 0.5]],
        pname="MAW-1",
    )
...
def _build_ts_shell(name, ws, thermal):
    """TDIS + GWF-MAW + GWT/GWE shell shared by the ts/peraux cases."""
    ...
    builder = _build_gwe_base if thermal else _build_gwt_base
...
def _build_cont_shell(name, ws, thermal, nper=3, well_rate=0.5):
    """TDIS + GWF-MAW + GWT/GWE shell shared by the persistence cases."""
    nlay, nrow, ncol, delr, delc, top, botm, idomain = _grid_params()
    aux_name = "TEMPERATURE" if thermal else "CONCENTRATION"

    sim = flopy.mf6.MFSimulation(sim_name=name, exe_name="mf6", sim_ws=ws)
    flopy.mf6.ModflowTdis(
        sim, time_units="DAYS", nper=nper, perioddata=[(1.0, 5, 1.0)] * nper
    )
    _build_gwf_default(
        sim,
        name,
...
def build_models_mwt_ts(name, ws):
    ...
    # STATUS=CONSTANT; CONCENTRATION and RATE link to TS; PACKAGEDATA
    # aux1 also links to TS
...
def build_models_mwe_ts(name, ws):
    ...
    # STATUS=CONSTANT; TEMPERATURE and RATE link to TS; PACKAGEDATA
    # aux1 also links to TS
...
def build_models_mwt_auxper(name, ws):
    ...
    sim, gwt, gwtname = _build_cont_shell(name, ws, thermal=False, nper=2)
    mwtperioddata = {
        0: [
            (0, "STATUS", "CONSTANT"),
...
def build_models_mwe_auxper(name, ws):
    ...
    sim, gwe, gwename = _build_cont_shell(name, ws, thermal=True, nper=2)
    mweperioddata = {
        0: [
            (0, "STATUS", "CONSTANT"),
            (0, "TEMPERATURE", 100.0),
...
def build_models_mwt_conccont(name, ws):
    ...
    """
    sim, gwt, gwtname = _build_cont_shell(name, ws, thermal=False, well_rate=-1.0)
...
def build_models_mwe_conccont(name, ws):
    ...
    sim, gwe, gwename = _build_cont_shell(name, ws, thermal=True, well_rate=-1.0)
...
def build_models_mwt_ratecont(name, ws):
    ...
    later period whose own PERIOD block doesn't mention it, GWT-MWT."""
    sim, gwt, gwtname = _build_cont_shell(name, ws, thermal=False, well_rate=Q_INJ)
...
def build_models_mwe_ratecont(name, ws):
    ...
    sim, gwe, gwename = _build_cont_shell(name, ws, thermal=True, well_rate=Q_INJ)
...
def _check_ts(bud_fname, obs_fname, pkg_label, conc_col, rate_col, thermal=False):
    """Shared T1+T2 check for MWT and MWE: verifies both the generic
    PERIOD TS re-sync and RATE's own per-timestep re-sync."""
    # ------------------------------------------------------------------ T1 --
