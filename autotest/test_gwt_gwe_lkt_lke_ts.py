"""LKT/LKE PERIOD time-series, PERIOD AUXILIARY, and cross-period
persistence integration tests.

  lkt_01ts/lke_01ts       -- PERIOD RAINFALL and PACKAGEDATA AUX both
                             linked to a time series that changes value
                             mid-period; both must track the new value
                             every timestep.
  lkt_01paux/lke_01paux   -- a literal PERIOD AUXILIARY value overrides
                             the PACKAGEDATA AUX baseline.
  lktraincont/lkeraincont -- a PERIOD RAINFALL value linked to a time
                             series keeps tracking it into a later period
                             whose own PERIOD block doesn't repeat it.
  lktscope                -- two independent lakes, each RAINFALL linked
                             to its own time series; reissuing lake 0's
                             PERIOD block leaves lake 1's still-active
                             link untouched.

lke_* cases reuse the GWT cases' GWF/LAK geometry with a GWE/LKE model in
place of GWT/LKT; lktscope uses its own two-lake grid.
"""
...
def build_models_lktraincont(name, ws):
    ...
    sim, gwt, gwtname = _build_cont_shell(name, ws, thermal=False)
...
def build_models_lkeraincont(name, ws):
    ...
    sim, gwe, gwename = _build_cont_shell(name, ws, thermal=True)
...
def _build_cont_shell(name, ws, thermal):
    """TDIS + GWF-LAK + GWT/GWE shell shared by the persistence cases."""
    nlay, nrow, ncol, delr, delc, top, botm, idomain = _grid_params()
    aux_name = "TEMPERATURE" if thermal else "CONCENTRATION"
    ...
    builder = _build_gwe_base_default if thermal else _build_gwt_base_default
    model, modelname = builder(
        sim, name, delr, delc, nlay, nrow, ncol, top, botm, idomain
    )
    return sim, model, modelname


def _build_ts_shell(name, ws, thermal):
    """TDIS + GWF-LAK + GWT/GWE shell shared by the ts/peraux cases."""
    nlay, nrow, ncol, delr, delc, top, botm, idomain = _grid_params()
    ...
    flopy.mf6.ModflowTdis(sim, time_units="DAYS", nper=1, perioddata=[(0.1, 10, 1.0)])
    gwf_builder = _build_gwf_lke if thermal else _build_gwf
    gwf_builder(
        sim,
        name,
        delr,
    ...
    base_builder = _build_gwe_base if thermal else _build_gwt_base
    model, modelname = base_builder(
        sim,
        name,
        delr,
        delc,
...
def build_models_ts(name, ws):
    ...
    sim, gwt, gwtname = _build_ts_shell(name, ws, thermal=False)
...
def build_models_lke_ts(name, ws):
    ...
    sim, gwe, gwename = _build_ts_shell(name, ws, thermal=True)
...
def _grid_params_2lake():
    """Grid for lktscope: two 3-connection lakes (cols 1-3, cols 5-7),
    separated by an idle gap cell, flanked by CHD boundaries."""
    lx = 9.0
    lz = 1.0
    nlay, nrow, ncol = 1, 1, 9
    delc = 1.0
    delr = lx / ncol
    delz = lz / nlay
    top = [0.0, 0.0, -0.90, 0.0, 0.0, 0.0, -0.90, 0.0, 0.0]
    botm = list(top - np.arange(delz, nlay * delz + delz, delz))
    botm[2] = -1.0
    botm[6] = -1.0
    idomain = np.full((nlay, nrow, ncol), 1)
...
def _build_gwf_2lake(
    sim, name, aux_name, delr, delc, nlay, nrow, ncol, top, botm, idomain
):
    """Build GWF model with two independent LAK lakes, mirrored geometry."""
    gwfname = "gwf_" + name
    gwf = flopy.mf6.MFModel(
        sim, model_type="gwf6", modelname=gwfname, model_nam_file=f"{gwfname}.nam"
    )
    ...
    flopy.mf6.modflow.ModflowGwflak(
        ...
        nlakes=2,
        noutlets=2,
        packagedata=[(0, -0.4, nlakeconn, 0.0), (1, -0.4, nlakeconn, 0.0)],
        connectiondata=[
            (0, 0, (0, 0, 1), "HORIZONTAL", DNODATA, 10, 10, connlen, connwidth),
            (0, 1, (0, 0, 2), "VERTICAL", DNODATA, 10, 10, connlen, connwidth),
            (0, 2, (0, 0, 3), "HORIZONTAL", DNODATA, 10, 10, connlen, connwidth),
            (1, 0, (0, 0, 5), "HORIZONTAL", DNODATA, 10, 10, connlen, connwidth),
            (1, 1, (0, 0, 6), "VERTICAL", DNODATA, 10, 10, connlen, connwidth),
            (1, 2, (0, 0, 7), "HORIZONTAL", DNODATA, 10, 10, connlen, connwidth),
        ],
        outlets=[
            (0, 0, -1, "SPECIFIED", 999.0, 999.0, 999.0, 999.0),
            (1, 1, -1, "SPECIFIED", 999.0, 999.0, 999.0, 999.0),
        ],
        perioddata=[
            (0, "STATUS", "CONSTANT"),
            (0, "STAGE", -0.4),
...
def build_models_lktscope(name, ws):
    """Case lktscope: reissuing lake 0's PERIOD block must not disturb
    lake 1's own still-active TS-linked RAINFALL, GWT-LKT."""
    nlay, nrow, ncol, delr, delc, top, botm, idomain = _grid_params_2lake()
    sim = flopy.mf6.MFSimulation(sim_name=name, exe_name="mf6", sim_ws=ws)
    flopy.mf6.ModflowTdis(
        sim, time_units="DAYS", nper=2, perioddata=[(1.0, 4, 1.0)] * 2
    )
    _build_gwf_2lake(
    ...
    lkt_obs = {
        (gwtname + ".lkt.obs.csv",): [
            ("lkt1-rain", "rainfall", 1),
            ("lkt2-rain", "rainfall", 2),
        ]
    }
    lkt = flopy.mf6.ModflowGwtlkt(
        gwt,
        ...
        save_flows=True,
        budget_filerecord=gwtname + ".lkt.bud",
        packagedata=[(0, 0.0, "mylkt1"), (1, 0.0, "mylkt2")],
        lakeperioddata={
            0: [
                (0, "STATUS", "CONSTANT"),
                (0, "CONCENTRATION", 0.0),
            ...
            1: [
                (0, "STATUS", "CONSTANT"),
                (0, "CONCENTRATION", 0.0),
                # lake 1 omitted entirely -- its TS-linked RAINFALL must
                # keep tracking rain1_ts
            ],
        },
        observations=lkt_obs,
        pname="LAK-1",
    ...
    lkt.ts.initialize(
        filename="rain.ts",
        timeseries=[(0.0, 10.0, 15.0), (1.0, 20.0, 25.0), (2.0, 20.0, 25.0)],
        time_series_namerecord=["rain0_ts", "rain1_ts"],
        interpolation_methodrecord=["stepwise", "stepwise"],
    )
    return sim, None


def check_output_lktscope(test):
    """Lake 1's RAINFALL obs must track rain1_ts in both periods, even
    though period 1's PERIOD block has no entry for lake 1 at all."""
    gwtname = "gwt_" + test.name
    fpth = test.workspace / f"{gwtname}.lkt.obs.csv"
    tc = np.genfromtxt(fpth, names=True, delimiter=",")

    lake0_obs = tc["LKT1RAIN"]
    lake1_obs = tc["LKT2RAIN"]
    assert len(lake0_obs) == 8 and len(lake1_obs) == 8, (
        f"Expected 8 obs per lake, got {len(lake0_obs)}/{len(lake1_obs)}"
    )

    exp_lake0 = [QRAIN * 10.0] * 4 + [QRAIN * 20.0] * 4
    exp_lake1 = [QRAIN * 15.0] * 4 + [QRAIN * 25.0] * 4
    assert np.allclose(lake0_obs, exp_lake0), (
        f"Lake 0 RAINFALL obs expected {exp_lake0}, got {lake0_obs}"
    )
    assert np.allclose(lake1_obs, exp_lake1), (
        f"Lake 1 RAINFALL obs expected {exp_lake1}, got {lake1_obs} -- "
        f"period 1's reissue of lake 0 must not disturb lake 1's own TS link"
    )
...
_BUILD_FNS = {
    ...
    "lktscope": build_models_lktscope,
}
...
_CHECK_FNS = {
    ...
    "lktscope": check_output_lktscope,
}
