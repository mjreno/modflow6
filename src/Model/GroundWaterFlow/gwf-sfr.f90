...
module SfrModule
  ...
  !> @brief Input context array pointers for SFR.
  !!
  !! PACKAGEDATA pointers are set in sfr_source_packagedata; PERIOD
  !! pointers are set once in sfr_ar.
  !<
  type :: SfrInputType
...
contains
  ...
  subroutine sfr_source_packagedata(this)
    ...
    do i = 1, this%maxbound
      ...
      !
      ! -- hk, roughness (Manning), upstream fraction: copied from
      !    input context; sfr_ad re-syncs TS-linked values each timestep
  ...
  subroutine sfr_source_diversions(this)
    ...
    do i = 1, isize
      ...
      if (d_iconr(i) < 1 .or. d_iconr(i) > this%maxbound) then
        write (errmsg, '(a,i0,a)') &
          'Diversion target reach number should be between 1 and ', &
          this%maxbound, '.'
        call store_error(errmsg)
        cycle
      end if
  ...
  subroutine sfr_ad(this)
    ...
    !    timestep; skipped when no TS6 files are configured
...
10  format('Diversion ', i0, ' of reach ', i0, &
  ...
  !!
  !! Shared by sfr_rp and sfr_ad; DIVERSION is handled separately by
  !! both callers because its target index requires its own validation.
  !<
  ...
  !!
  !! Called once from sfr_ar; PACKAGEDATA pointers are set separately
