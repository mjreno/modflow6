...
\item The Hydraulic Flow Barrier (HFB) Package \citep{hsieh1993hfb, modflow2005} in \mf allows barrier properties and locations to change by stress period.  The capability to change barriers by stress period was not supported in previous MODFLOW versions.
...
\item There are two different forms of input for the RCH, EVT, and SPC Packages: array-based input and list-based input. Similarly, there are two different forms of input for the CHD, DRN, GHB, RIV and WEL Packages: ``grid array''-based input and list-based input.  Whereas array-based input directs \mf to read a single two-dimensional array of input values, the use of grid arrays input directs \mf to read as many two-dimensional arrays as there are grid layers.  For models that use the DIS Package, input for these packages can be provided as arrays, which is consistent with previous MODFLOW versions.  To use array input, the user must specify the READASARRAYS or READARRAYGRID keywords, as appropriate, in the options block.  The READASARRAYS or READARRAYGRID options can also be used for models that use the DISV Package.  If the READASARRAYS or READARRAYGRID options are not specified, then input to the aforementioned packages is provided in list form.  List-based input is the only option supported when the DISU Package is used.
...
\item Similarly, the CHD, WEL, DRN, RIV, and GHB Packages also support two forms of input: list-based input and grid array-based input.  Grid array-based input is activated by specifying the READARRAYGRID keyword in the options block.  When READARRAYGRID is specified, stress values are provided as grid-dimensioned arrays rather than as lists of individual cells.  Cells containing the DNODATA value (3.0E+30) are treated as inactive for the stress package.  The package type in the GWF Model name file PACKAGES block remains unchanged (e.g., ``CHD6'', ``WEL6'') regardless of which input form is used.
