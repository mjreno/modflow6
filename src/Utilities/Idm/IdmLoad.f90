...
contains
  ...
  recursive subroutine input_load(component_type, subcomponent_type, modelname, &
    ...
    do n = 1, static_loader%subpkg_list%pnum
      ...
      call mem_allocate(subpkg_mempath, LENMEMPATH, &
                        static_loader%subpkg_list%varnames(n), &
                        static_loader%mf6_input%mempath)
      !
      ! -- create and set the mempath
      subpkg_mempath = &
        create_mem_path(static_loader%mf6_input%component_name, &
