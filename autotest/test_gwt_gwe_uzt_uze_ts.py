"""UZT/UZE PERIOD time-series, PERIOD AUXILIARY, and cross-period
persistence integration tests.

Covers GWT-UZT and GWE-UZE in paired cases sharing the same GWF+UZF model.

  PERIOD TS: CONCENTRATION/TEMPERATURE (CONSTANT status) and INFILTRATION
             linked to a STEPWISE TS that steps mid-period, applied to
             the one cell not CHD-adjacent (the only one with measurable
             INFILTRATION); both must track the new value every timestep.
  ...
  Cross-period persistence: a PERIOD INFILTRATION value linked to a TS
             keeps tracking it into a later period whose own PERIOD
             block reappears (for STATUS/CONCENTRATION) without
             repeating INFILTRATION. Actual infiltration isn't a simple
             prescribed constant (variably saturated flow accounting can
             adjust it relative to FINF), so this case derives its
             expected scale from period 1's own observed value, then
             checks periods 2/3 track the TS ratios (2x, 3x) relative
             to that.
...
def _build_ts_shell(name, ws, thermal):
    """TDIS + GWF-UZF + GWT/GWE shell shared by the ts/peraux cases."""
    ...
    _build_gwf(sim, name, aux_name, delr, delc, nlay, nrow, ncol, top, botm, idomain)
    ...
    return sim, model, modelname, ncol


def _build_cont_shell(name, ws, thermal):
    """TDIS + GWF-UZF + GWT/GWE shell shared by the persistence cases."""
    nlay, nrow, ncol, delr, delc, top, botm, idomain = _grid_params()
    aux_name = "TEMPERATURE" if thermal else "CONCENTRATION"
...
def build_models_uztinflcont(name, ws):
    ...
    GWT-UZT."""
    sim, gwt, gwtname, ncol = _build_cont_shell(name, ws, thermal=False)
    nuzfcells = ncol
...
def build_models_uzeinflcont(name, ws):
    ...
    sim, gwe, gwename, ncol = _build_cont_shell(name, ws, thermal=True)
    nuzfcells = ncol
...
def build_models_uzt_ts(name, ws):
    ...
    sim, gwt, gwtname, ncol = _build_ts_shell(name, ws, thermal=False)
    nuzfcells = ncol
    ...
    # STATUS=CONSTANT; CONCENTRATION and INFILTRATION link to TS. Feature 2
    # (0-based ifno=1) is the only cell not CHD-adjacent, so it's the one
    # with a measurable INFILTRATION flow.
    uzt = flopy.mf6.modflow.ModflowGwtuzt(
        gwt,
        boundnames=True,
        save_flows=True,
...
def build_models_uze_ts(name, ws):
    ...
    sim, gwe, gwename, ncol = _build_ts_shell(name, ws, thermal=True)
    nuzfcells = ncol
...
def _check_ts(bud_fname, obs_fname, pkg_label, conc_col, infl_col, thermal=False):
    """Shared T1+T2 check for UZT and UZE: verifies both the generic
    PERIOD TS re-sync and INFILTRATION's own per-timestep re-sync."""
    # ------------------------------------------------------------------ T1 --
