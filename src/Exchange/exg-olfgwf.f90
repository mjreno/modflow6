!> @brief This module contains the OlfGwfExchangeModule Module
!!
!! This module contains a lightweight OLF-GWF exchange class which is
!! primarily based on the underlying and generic SWF-GWF code for connecting
!< a SWF model (either CHF or OLF) with a GWF model.
module OlfGwfExchangeModule
...
contains
  ...
  !! GWF model.
  !<
  subroutine olfgwf_cr(filename, name, id, m1_id, m2_id, input_mempath)
    ! modules
    ! dummy
    ...
    ! local
    type(OlfGwfExchangeType), pointer :: exchange
    ...
    call exchange%initialize(filename, name, 'OLF', id, m1_id, m2_id, &
                             input_mempath)

  end subroutine olfgwf_cr
  ...
  subroutine olf_gwf_df(this)
    !  modules
    !  dummy
    class(OlfGwfExchangeType) :: this !<  SwfGwfExchangeType
    !  local
  ...
  subroutine source_options(this, iout)
    ...
    write (iout, '(1x,a)') 'Processing OLF-GWF exchange options'
    ...
    write (iout, '(1x,a)') 'End of OLF-GWF exchange options'
...
end module OlfGwfExchangeModule
