...
def build_period_precedence(test):
    ...
    # period 1 gets 2 steps to confirm the override holds across a later
    # timestep in the same period it was set, not just eventually
    flopy.mf6.ModflowTdis(
        sim, nper=3, perioddata=[(1.0, 1, 1.0), (1.0, 2, 1.0), (1.0, 1, 1.0)]
    )
    flopy.mf6.ModflowIms(sim, print_option="NONE")
...
def check_period_precedence(test):
    ...
    flow0, flow1a, flow1b, flow2 = (float(obs["OUT1"][i]) for i in range(4))
    assert not np.isclose(flow0, flow1a), (
    ...
    assert np.isclose(flow1a, flow1b, rtol=1e-6), (
        "the override should still hold on period 2's second timestep, not "
        f"just its first; got flow1a={flow1a}, flow1b={flow1b}"
    )
    assert np.isclose(flow1b, flow2, rtol=1e-6), (
        ...
        f"own resync to its next TS value; got flow1b={flow1b}, flow2={flow2}"
    )
