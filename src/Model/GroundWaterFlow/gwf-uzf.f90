...
contains
  ...
  subroutine uzf_allocate_arrays(this)
    ...
    !
    ! -- input context pointers
    call mem_setptr(this%sinf_pvar, 'FINF', this%input_mempath)
    call mem_setptr(this%pet_pvar, 'PET', this%input_mempath)
    call mem_setptr(this%extdp, 'EXTDP', this%input_mempath)
    call mem_setptr(this%extwc_pvar, 'EXTWC', this%input_mempath)
    call mem_setptr(this%ha_pvar, 'HA', this%input_mempath)
    call mem_setptr(this%hroot_pvar, 'HROOT', this%input_mempath)
    call mem_setptr(this%rootact_pvar, 'ROOTACT', this%input_mempath)
    call mem_setptr(this%uauxvar, 'AUXVAR', this%input_mempath)
    !
    ! -- initialize
    do i = 1, this%nodes
      ...
      ! -- integer variables
      this%ja(i) = 0
      ! -- sinf_pvar/pet_pvar/extdp/extwc_pvar/ha_pvar/hroot_pvar/
      ! -- rootact_pvar are input context pointers
      do j = 1, this%naux
  ...
  subroutine uzf_rp(this)
    ...
    if (this%iper == kper) then
      ...
      do n = 1, nlist
        ...
        !
        ! -- sinf_pvar/pet_pvar/extdp/extwc_pvar/ha_pvar/hroot_pvar/
        ! -- rootact_pvar/uauxvar alias finf/pet/extdp/extwc/ha/hroot/
        ! -- rootact/AUX directly; nothing to merge
        !
        ! -- write table output
        if (this%iprpak /= 0) then
  ...
  subroutine uzf_ad(this)
    ...
    ! -- sinf_pvar/pet_pvar/extdp/extwc_pvar/ha_pvar/hroot_pvar/
    ! -- rootact_pvar/uauxvar alias finf/pet/extdp/extwc/ha/hroot/
    ! -- rootact/AUX directly; nothing to re-sync
    !
  ...
  subroutine uzf_source_packagedata(this)
    ...
    call memorystore_release('PACKAGEDATA_IFNO', this%input_mempath)
    call memorystore_release('CELLID', this%input_mempath)
    call memorystore_release('LANDFLAG', this%input_mempath)
    call memorystore_release('IVERTCON', this%input_mempath)
    call memorystore_release('SURFDEP', this%input_mempath)
    call memorystore_release('VKS', this%input_mempath)
    call memorystore_release('THTR', this%input_mempath)
    call memorystore_release('THTS', this%input_mempath)
    call memorystore_release('THTI', this%input_mempath)
    call memorystore_release('EPS', this%input_mempath)
...
30  format(i10)
  ...
  subroutine uzf_da(this)
    ...
    !
    ! -- sinf_pvar/pet_pvar/extdp/extwc_pvar/ha_pvar/hroot_pvar/
    ! -- rootact_pvar/uauxvar are input context pointers
    !
    ! -- Parent object
