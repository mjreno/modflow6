...
error message expected in mfsim.lst:
  ...
  sfr-iconr-error  out-of-range diversion target reach
  sfr-aux-ifno     PERIOD AUXILIARY targeting an out-of-range reach
"""
...
def _build_cd_ifno_mismatch(test, name):
    ...
    # 3-reach chain (PACKAGEDATA IFNOs 0-2); CONNECTIONDATA's middle row
    # uses IFNO 5, a typo for 1 with no matching PACKAGEDATA entry
    packagedata = [
...
def _build_div_iconr_error(test, name):
    sim = flopy.mf6.MFSimulation(sim_name=name, exe_name="mf6", sim_ws=test.workspace)
    flopy.mf6.ModflowTdis(sim, nper=1, perioddata=[(1.0, 1, 1.0)])
    flopy.mf6.ModflowIms(sim, print_option="NONE")
    gwf = flopy.mf6.ModflowGwf(sim, modelname=name, save_flows=True)
    ...
    # two reaches, reach 0 diverts to reach 5, which does not exist
    packagedata = [
        [0, (0, 0, 0), 100.0, 5.0, 1e-3, 4.0, 1.0, 1e-5, 0.04, 1, 1.0, 1],
        [1, (0, 0, 1), 100.0, 5.0, 1e-3, 4.0, 1.0, 1e-5, 0.04, 1, 1.0, 0],
    ]
    connectiondata = [[0, -1], [1, 0]]
    diversions = [[0, 0, 5, "FRACTION"]]  # target reach 5 is out of range
    perioddata = {0: [[0, "inflow", 0.1], [0, "diversion", 0, 0.5]]}
    flopy.mf6.ModflowGwfsfr(
        gwf,
        nreaches=2,
...
def _build_aux_ifno_error(test, name):
    sim = flopy.mf6.MFSimulation(sim_name=name, exe_name="mf6", sim_ws=test.workspace)
    flopy.mf6.ModflowTdis(sim, nper=1, perioddata=[(1.0, 1, 1.0)])
    flopy.mf6.ModflowIms(sim, print_option="NONE")
    gwf = flopy.mf6.ModflowGwf(sim, modelname=name, save_flows=True)
    ...
    # two reaches; PERIOD AUXILIARY targets reach 6, which does not exist
    packagedata = [
        [0, (0, 0, 0), 100.0, 5.0, 1e-3, 4.0, 1.0, 1e-5, 0.04, 1, 1.0, 0, 0.0],
        [1, (0, 0, 1), 100.0, 5.0, 1e-3, 4.0, 1.0, 1e-5, 0.04, 1, 1.0, 0, 0.0],
    ]
    connectiondata = [[0, -1], [1, 0]]
    perioddata = {0: [[0, "inflow", 0.1], [5, "AUXILIARY", "conc", 99.0]]}
    flopy.mf6.ModflowGwfsfr(
        gwf,
        auxiliary=["conc"],
        nreaches=2,
...
_builders = [
    ...
    _build_obs_badreach,
    _build_cprior_error,
    _build_div_iconr_error,
    _build_aux_ifno_error,
]
...
_expected_lst_substrings = [
    ...
    ["Invalid cprior type"],
    ["Diversion target reach number should be between 1 and 2."],
    ["IFNO 6 on row 2 must be greater than 0 and less than or equal to 2."],
]
