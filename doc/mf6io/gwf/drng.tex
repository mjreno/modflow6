Input to the Drain (DRNG) Package is read from the file that has type ``DRN6'' in the Name File.  Any number of DRN6 packages can be specified for a single groundwater flow model.  The package type in the GWF Model name file PACKAGES block is always ``DRN6'' regardless of whether list-based or grid array-based input is used; the READARRAYGRID option within the package input file activates grid array-based input.

When grid array-based input is used for the drain package, the DIMENSIONS block is optional.  The input array size is determined from the model grid.  Cells that define no data should contain the DNODATA (3.0E+30) value.  When defined, the MAXBOUND dimension represents the maximum number of defined drain (non-DNODATA) cells found in any stress period.  Defining MAXBOUND can reduce the memory footprint of the grid array-based DRNG Package, especially when the input grid arrays are sparse and the model grid is large.  Grid array-based input for drains can be used with the DIS and DISV Packages.
