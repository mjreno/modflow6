...
Input to the Constant-Head (CHDG) Package is read from the file that has type ``CHD6'' in the Name File.  Any number of CHD6 packages can be specified for a single groundwater flow model; however, an error will occur if a CHD6 package attempts to make a GWF cell a constant-head cell when that cell has already been designated as a constant-head cell either within the present CHD6 package or within another CHD6 package.  The package type in the GWF Model name file PACKAGES block is always ``CHD6'' regardless of whether list-based or grid array-based input is used; the READARRAYGRID option within the package input file activates grid array-based input.

When grid array-based input is used for constant-head boundaries, the DIMENSIONS block is optional.  The input array size is determined from the model grid.  Cells that define no data should contain the DNODATA (3.0E+30) value.  When defined, the MAXBOUND dimension represents the maximum number of defined constant-head boundary (non-DNODATA) cells found in any stress period.  Defining MAXBOUND can reduce the memory footprint of the grid array-based CHDG Package, especially when the input grid arrays are sparse and the model grid is large.  Grid array-based input for constant-head boundaries can be used with the DIS and DISV Packages.
...
CHD Package observations are limited to the simulated constant head flow rate (\texttt{chd}). The data required for the CHD Package observation type is defined in table~\ref{table:gwf-chdgobstype}. Negative and positive values for an observation represent a loss from and gain to the GWF model, respectively.
